...
Conversation routing preserves recent references and saved task context across topic changes. It improved the measured follow-up and task-resumption cases, while casual replies can still mention unrelated work. See [the design and evaluation report](docs/conversation-routing.md) for results, latency and known limitations.
...
Memory lives in `.private/memory/transcript.json` and `.private/memory/memories.json`, outside Git. Timestamps stay in UTC on disk and are shown to the agent in Beijing time with an explicit offset. The runtime keeps six recent exchanges and durable source references for the current topic and most recent task. A small router distinguishes social conversation, follow-ups, topic changes and dialogue questions. Exact standalone greetings omit history for that reply; model routing cannot remove the recent window. Stable core facts are injected, while progress and open issues remain available through the memory tool. After replies, a separate DeepSeek call extracts durable facts when the chat is idle for 30 seconds, after five pending turns, or immediately after an explicit “remember” request. Failed extraction leaves the original dialogue available for retry. The `memory` tool searches curated facts first and can search archived wording when the agent needs older detail.
