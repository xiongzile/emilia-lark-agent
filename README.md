...
- Gives the agent the current date, time, and time zone for each turn, and logs model token/cache usage, estimated cost, and first-token latency.
...
- Organizes conversation into inspectable topic trees backed by Pi JSONL sessions. Original messages and tool outcomes survive topic changes, corrections, compaction and restart.
- Distills durable facts into categorized JSON memory; provides text commands and a `context_tree` tool to search, restore, move and undo topic membership.
- Bounds CLI output with local full-result files and paged reads, preserves a stable conversation prefix, and summarizes older history when its token budget is reached.
- Includes a macOS `launchd` service with automatic restart and `caffeinate -i` support.

Jev selects a topic and a `greet`, `chat`, or `task` response mode. The selected topic determines context; changing response mode does not cut off its history. Greetings start a clean topic. The agent can search and restore earlier topics through `context_tree`. See [context-tree architecture](docs/conversation-routing.md) and [evaluation](docs/context-architecture.md).
...
Pi stores messages, tool calls/results, topic references and per-topic checkpoints under `.private/memory/sessions/`. Curated facts remain in `.private/memory/memories.json`. On first open, the old `transcript.json` is backed up as `transcript.legacy.json` and imported with its original IDs/timestamps into a labelled history topic. Missing historical tool evidence is not invented. After import, the old JSON is no longer read or updated. Keep `.private/` local: it contains private conversations and tool output.

Working context is summarized at a 64,000-token soft threshold while retaining about 16,000 recent tokens and the complete active turn. Restart restores topic membership, undo history, checkpoints and following messages, including recorded tool outcomes; it does not repeat tool calls. Summaries do not delete original history. See [context and cache management](docs/context-cache.md).

Send `/tree` to inspect topics, `/tree show <id>` to read a topic or turn, and `/tree context` to inspect the latest actual model request. `/tree search <words>`, `/tree use <id>`, `/tree new [title]`, `/tree move <turn-id> <topic-id>` and `/tree undo` provide direct recovery and correction. Natural-language corrections can invoke the same operations through `context_tree`. No GUI is required.

Historical observations describe that time, not current external state. The `memory` tool handles distilled facts separately. Chat receives profile/preferences; work also receives stable project facts and decisions. Progress and open issues remain available through memory search.

After replies, a separate DeepSeek call extracts durable facts when the chat is idle for 30 seconds, after five pending turns, or immediately after an explicit “remember” request. Failed extraction leaves original dialogue available for retry.

Send `/memory` for counts and extraction status, `/memory list [category]` to inspect facts, `/memory show <id>` for provenance, `/memory update` to process pending dialogue and compare facts with recent chat, or `/memory forget <id>` to remove a fact. Forgetting a fact does not delete original history. The update command does not claim to verify external repositories or Lark resources it has not inspected.
...
Set `JEV_API_KEY` in the ignored `.env` for topic and mode classification (the `TYPESAFE_API_KEY` alias also works). The router sends the current message, ten complete recent exchanges and previews of twelve recent topics to TypeSafe, using pinned `jev-1.13.0`. DeepSeek handles replies, tools and memory distillation. Unavailable routing retains the active topic and nearby originals; it can expose more history than a casual reply needs. `context_tree` remains available in every mode. No extra SDK is required.
...
Configured commands use `execFile` without a shell. Lark, Git, and configured CLIs share a bounded output path: previews are at most 16 KiB/400 lines/12,000 characters, and complete captured output is saved under `.private/tool-output/` for paged reads through `workspace_files`. File reads accept `offset` and `limit`; files up to 16 MiB can be read in pages. Subprocess `maxOutputBytes` remains a separate capture limit.

Policies can restrict the first argument, individual arguments, consecutive argument sequences, and regular-expression matches. This provides a useful boundary but does not turn an unsafe executable into a sandbox; expose narrowly scoped CLIs and keep their authentication outside the agent.
