...
// JSON archives contain final wording, not tool execution evidence. Live turns
// instead reuse their original message group, including paired tool calls/results.
export function restoreTurn(turn: Turn, model: Model<Api>): AgentMessage[] {
    const timestamp = Date.parse(turn.at);
    return [
        {role: "user", content: `[历史消息时间：${formatAgentTime(turn.at)}]\n${turn.user.slice(0, 2000)}`, timestamp},
        {
